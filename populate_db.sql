...
-- Create tables
create table `User` (id int primary key auto_increment, nickname tinytext not null, email tinytext not null, fname tinytext not null, lname tinytext not null, birthDate DATETIME not null);
...
alter table `User` add (numAPI_key int, numCharacter int, numRank int, numDenunciation int, foreign key (numAPI_key) references `API_key`(id), foreign key (numCharacter) references `Character`(id), foreign key (numRank) references `Rank`(id), foreign key (numDenunciation) references Denunciation(id));

alter table Message add (numUser int, foreign key (numUser) references User(id));